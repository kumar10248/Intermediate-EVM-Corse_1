...
contract ErrorHandling {
    ...
    modifier Teacher() {
        require(
            msg.sender == teacher,
            "Only the teacher can Give the grade to the Student."
        );
        _;
    }
    ...
    function getGrade(address student) public view returns (string memory) {
        ...
        require(
            bytes(grade).length > 0,
            "Student does not have a grade assigned."
        );
    ...
    function otherTeacher(address newTeacher) public Teacher {
        ...
        require(
            newTeacher != address(0),
            "New teacher's address must be valid."
        );
